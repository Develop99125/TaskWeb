...
<html>
<head>
    <title>Мои изображения</title>
</head>
<body>

<span>
        <input type="button" value="<<" onclick="window.location.href = '/my/images/page/1'"/>
        <input type="button" value="<" onclick="window.location.href = '/my/images/page/${pagInfo.currentPage - 1}'"/>
        ...
        <c:forEach var="pageNumber" items="${pagInfo.pages}">
            <c:if test="${pageNumber == pagInfo.currentPage}">
                <input type="button" style="background-color: aquamarine" value="${pageNumber}"
                       onclick="window.location.href = '/my/images/page/${pageNumber}' "/>
            </c:if>
            <c:if test="${pageNumber != pagInfo.currentPage}">
        ...
        <input type="button" value=">" onclick="window.location.href = '/my/images/page/${pagInfo.currentPage + 1}'"/>
        <input type="button" value=">>" onclick="window.location.href = '/my/images/page/${pagInfo.totalPages}'"/>
    </span>

<input type="button" value="Добавить изображение"
       onclick="window.location.href = '/images/add'"
/>
<input type="button" value="Мои изображения" style="background-color: aquamarine"
...
<table>
    <c:forEach var="image" items="${images}">
        <tr>
            <td>Номер изображения: ${image.id}</td>
